...
## 13. 작업 보고 검증 — 7차 (2026-07-12 18:53 KST)

검증 대상: commit `4a28d84` (`fix(security): v4/v5/v6 feedback — timing defense, email migration, ws bounds`)와 cleanup commit `406f3b0`, 갱신된 `outbox/report.md`

확인 결과:

- dummy Argon2 verify가 unknown/OAuth-only login에도 core gate를 통과하므로 즉시 응답 timing oracle은 제거됐다.
- login email·password validation이 실제 core 경로에 적용됐다.
- migration은 canonical 충돌 시 자동 병합하지 않고 실패하며, backfill 후 functional unique index를 생성한다. 안전한 기본 정책이다.
- WS는 pending auth socket을 포함한 상한, room 총량·사용자별 생성률, pre-upgrade Origin 거부를 구현했다. channel 존재 확인만 남았다.
- `Fly-Client-IP` 선택은 Fly HTTP Proxy가 해당 header를 추가한다는 공식 문서와 부합한다. 다른 reverse proxy가 Fly 앞에 추가되면 proxy IP가 들어간다는 제한은 유지된다. 참고: https://fly.io/docs/networking/request-headers/
- 아래 운영·검증 결함을 보완해야 한다.

### V7-1. production deploy workflow가 email migration을 실행하지 않음 — P0

근거:

- `.github/workflows/deploy.yml`은 svc-core image를 배포하지만 DB migration step이 없다.
- `docs/DEPLOY.md`에는 service 배포 후 수동 `db:migrate`가 적혀 있다.
- 새 core는 canonical email 동작을 즉시 사용하지만 DB canonical constraint는 migration 전에는 없다.
- migration은 기존 충돌이 있으면 의도적으로 실패한다. service 배포 후 실패하면 application과 DB policy가 불일치한 상태가 된다.

영향:

production에 code만 배포되고 migration이 누락될 수 있다. 또는 신규 core 배포 후 migration 충돌로 중단되어 lowercase 계정 선점 창이 다시 열린다.

지시:

1. production data에 canonical collision이 없는지 read-only preflight를 먼저 실행한다.
2. 충돌이 있으면 배포를 중단하고 수동 해결한다. 자동 병합하지 않는다.
3. migration과 application compatibility를 고려한 배포 순서를 명시한다. 필요하면 “canonical login 호환 코드 → migration → canonical register 강제”의 2단계 release로 분리한다.
4. deploy workflow 또는 Fly release command에서 migration 성공을 svc-core release 전제조건으로 만든다.
5. migration 실패 시 새 svc-core 배포를 진행하지 않고, 성공 여부와 schema version을 기록한다.

### V7-2. ingest Origin allowlist의 comma-only 값이 production 검증을 우회함 — P1

근거:

- production 검증은 raw `INGEST_ALLOWED_ORIGINS.trim()`이 비었는지만 확인한다.
- runtime은 comma split 후 빈 항목을 제거한다.
- 값이 `","` 또는 `" , "`이면 production 검증을 통과하지만 runtime `allowedOrigins`는 빈 배열이 되어 Origin 검사를 비활성화한다.

지시:

- env schema에서 split·trim·filter한 최종 배열이 production에서 최소 1개인지 검증한다.
- 각 항목을 `new URL()`로 parse하고 `https:` production origin만 허용한다.
- origin에는 path, query, fragment, wildcard, credential이 없어야 한다.
- comma-only, invalid URL, `https://good.example.evil.test`, trailing slash 정책을 test한다.
- 동일 parser 결과를 env 검증과 ingest runtime이 공유해 해석 차이를 없앤다.

### V7-3. CI smoke 실패 시 service log step이 실행되지 않음 — P1

근거:

- `Service logs on failure` step은 `Session smoke` step보다 앞에 있다.
- 해당 시점까지 startup이 성공했으면 `if: failure()`는 false라 log step이 skip된다.
- 이후 session smoke가 실패해도 이미 지난 step은 다시 실행되지 않는다.

지시:

- log step을 session smoke 뒤로 옮기고 `if: failure()` 또는 `if: always() && failure()`를 적용한다.
- startup loop가 core port를 30초 내 열지 못했을 때 명시적으로 실패하는 조건도 추가한다. 현재 loop 종료 후에도 BFF startup을 계속한다.
- background PID를 저장하고 마지막 cleanup step에서 종료한다.

### V7-4. timing 방어 완료 판정에는 분포 검증이 필요 — P1

근거:

- 보고된 표본은 wrong-password 76ms, unknown 59ms 한 번씩이다.
- 즉시 실패 oracle은 줄었지만 단일 표본으로 timing indistinguishability를 입증할 수 없다.
- 실제 사용자 hash는 과거 parameter를 가질 수 있고 dummy hash는 현재 default parameter다.

지시:

- warm-up 후 각 경로 수백 회 latency를 수집해 median, p95, 분포를 비교한다.
- 기존 hash parameter variation이 있는지 조사한다.
- 차이가 지속되면 로그인 성공 여부와 무관하게 동일 policy로 password hash를 점진 재해시하는 계획을 둔다.

### V7-5. module-scope dummy hash 초기화 실패 처리 — P2

`dummyHashPromise`가 module import 시 시작되며 명시적으로 startup에서 await되지 않는다. Argon2 초기화가 실패하면 login 전 unhandled rejection으로 process가 불명확하게 종료될 수 있다. core bootstrap 단계에서 dummy hash 준비를 await하고 실패 시 명확한 fatal startup error로 종료하도록 개선한다.

### V7-6. channel 존재 확인은 여전히 필요 — P1

room 수와 생성률이 bounded되어 즉시 고갈 위험은 줄었다. 그러나 유효 token 사용자가 존재하지 않는 UUID room을 만들 수 있다는 V4-1 본질은 남아 있다. by-id/internal validation RPC 추가 전 P2-2를 완전 완료로 표시하지 않는다.

### 7차 판정

- V5-1 timing 방어: **구현 완료, 통계 검증 대기**.
- V5-2 env 범위: **완료**.
- V4-1 room 경계: **부분 완료** — resource bounded, 존재 확인 미완료.
- V4-2/V4-3: **완료**.
- V6-3 email canonicalization: **코드·migration 완료, production 배포 절차 미완료**.
- V6-4 login validation: **완료**.
- V6-1 client IP: **공식 header 사용 완료, staging 확인 대기**.
- V6-6 CI 안정성: **부분 완료** — readiness/log step 결함 잔여.

다음 우선순위: V7-1 migration 배포 gate → P2-5 dependency update/audit → V7-2 origin parser → V7-3 CI log 순서 → channel 존재 RPC.

## 14. 작업 보고 검증 — 8차 (2026-07-12 18:58 KST)

검증 대상: commit `6b9c003` (`fix(deps): resolve production advisories — drizzle-orm 0.45.2, postcss 8.5.16`), 갱신된 `outbox/report.md`

확인 결과:

- `drizzle-orm`이 모든 직접 importer에서 0.45.2로 통일됐다.
- `drizzle-kit` 0.31.10 업데이트와 migration·CRUD·session 회귀 결과가 보고됐다.
- lockfile에서 취약한 PostCSS 8.4.31이 제거되고 8.5.16으로 단일화됐다.
- production audit 결과가 clean이며 high 이상 CI gate가 추가됐다.
- P2-5의 알려진 두 advisory는 현재 lockfile 기준 해소된 것으로 판정한다.

### V8-1. PostCSS override 범위가 future major까지 허용할 수 있음 — P2

근거:

- root override 값은 `">=8.5.10"`이다.
- 향후 lockfile 재생성 시 호환성이 검증되지 않은 다음 major를 선택할 수 있는 열린 범위다.

지시:

- 임시 override를 정확한 검증 버전 `8.5.16` 또는 `<9`로 제한된 compatible range로 고정한다.
- Next가 안전한 PostCSS를 직접 의존하는 버전으로 올라가면 override를 제거한다.
- override 제거 가능 여부를 정기 dependency update에서 확인한다.

### V8-2. production deploy가 mutable GitHub Action `master`를 실행함 — P0

근거:

- `.github/workflows/deploy.yml`은 `superfly/flyctl-actions/setup-flyctl@master`를 사용한다.
- deploy job의 `FLY_API_TOKEN`은 job-level env라 모든 step/action process에서 접근 가능하다.
- `master`는 고정된 artifact가 아니며 upstream 변경에 따라 실행 코드가 바뀐다.

영향:

upstream action compromise 또는 예기치 않은 변경이 Fly token을 탈취하고 production app을 임의 배포할 수 있다.

지시:

1. action을 검증한 full commit SHA로 pin하고 주석에 release tag를 기록한다.
2. `FLY_API_TOKEN`을 job 전체가 아니라 실제 `flyctl deploy` step에만 scope한다.
3. token 권한과 수명을 최소화하고 정기 rotation한다.
4. Dependabot/Renovate가 action SHA update PR을 만들게 하고 diff를 검토한다.

### V8-3. production deploy가 `vercel@latest`를 token과 같은 step에서 설치함 — P0

근거:

- deploy workflow는 `npm i -g vercel@latest` 후 같은 step에서 `VERCEL_TOKEN`을 사용한다.
- `latest`가 가리키는 package 내용과 transitive dependency는 실행 시점마다 바뀔 수 있다.
- step env의 Vercel token은 package install·lifecycle process에서도 접근 가능하다.

영향:

npm package 공급망 compromise가 production Vercel token 탈취와 악성 web 배포로 이어질 수 있다.

지시:

- 검증한 Vercel CLI 버전을 repository dependency/lockfile에 pin하고 frozen install 결과만 실행한다.
- 설치 step과 token 사용 step을 분리해 install process에는 token을 제공하지 않는다.
- 가능하면 Vercel 공식 pinned action 또는 검증된 standalone binary checksum 방식을 사용한다.
- Vercel token scope를 해당 project·production deploy에 필요한 최소 권한으로 제한한다.

### V8-4. container base·service image도 mutable tag 사용 — P1

근거:

- Dockerfile은 `node:22-slim`, MediaMTX `latest`를 사용한다.
- docker-compose는 `minio/minio:latest`, `bluenviron/mediamtx:latest`를 사용한다.

영향:

동일 commit을 다시 build해도 다른 binary가 포함될 수 있다. upstream compromise·breaking update·재현 불가능한 rollback 위험이 있다.

지시:

- production Dockerfile image는 검증한 version과 digest로 pin한다.
- digest update는 자동 PR로 받고 changelog·취약점·smoke를 검토한다.
- local compose도 가능하면 version tag를 고정해 개발 재현성을 높인다.
- build provenance와 최종 image digest를 release 기록에 남긴다.

### V8-5. audit gate 실행 위치와 범위 — P2

- audit는 build 뒤보다 install 직후 실행하면 취약 dependency에서 빠르게 실패한다.
- high gate는 적절하다. moderate는 현재처럼 정기 triage하되 결과를 무기한 무시하지 않도록 issue/Dependabot 정책을 둔다.
- `pnpm audit` registry 장애와 advisory DB 한계를 고려해 Dependabot 또는 OSV scanner를 보조 신호로 추가한다.

### 8차 판정

- P2-5 dependency advisory: **완료**.
- CI dependency gate: **완료, 보조 scanner 선택 사항**.
