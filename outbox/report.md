...
# 27차 반복 (2026-07-12) — 취약 비밀번호 차단 (P2-4)

review.md 신규 갱신 없음. 검증자 P2-4 "유출 비밀번호 차단, password manager 허용".
외부 API 없이 자체완결(번들 common + 패턴).

## 완료

### 취약 비밀번호 거부 — 완료

- `packages/schemas/src/weak-password.ts`: `isWeakPassword()` — 반복 단일문자,
  단조 숫자 순열, 번들 common 12+자 문구(대소문자 무관) 거부. 외부 호출 없음.
- `passwordSchema`에 refine 연결 → register + change-password 신규 비밀번호에
  자동 적용. min12는 이미 짧은 고전 비밀번호 차단, 이건 12+자 취약 패턴 대상.
- vitest 7케이스(schemas 12/12).

password manager 친화 유지(복잡도 규칙 없음, 길이+취약패턴만).

## 검증 결과 (27차)

- schemas test 12/12, core lint·build·web build 통과
- **실검증**: register `aaaaaaaaaaaa`/`123456789012` → 400,
  강한 비밀번호 → 201.
